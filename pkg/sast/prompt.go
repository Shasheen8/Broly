...
## Common AI Anti-Patterns to Watch For:

These patterns are frequently generated by AI tools and commonly missed by automated checks:
- **SQL/NoSQL injection via string concatenation** -- f-strings, template literals, or + operators building queries
- **Command injection** -- user input passed to shell execution functions without argument arrays
- **XSS** -- innerHTML, template rendering without context-specific output encoding
- **Hardcoded secrets** -- API keys, passwords, JWT secrets in source code or config files
- **Weak cryptography** -- MD5/SHA1 for security, ECB mode, insecure random for tokens
- **Path traversal** -- user input concatenated into file paths without canonicalization
- **Missing input validation** -- no type/length/format checks on user-controllable data before use
- **JWT misuse** -- missing algorithm enforcement, weak secrets, tokens in localStorage
`
