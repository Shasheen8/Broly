...
**Status:** v1.0.0 tagged and released. Phases 1-8C, 11B, 11D complete. 11C deferred. Only 11A remains before Togethercomputer org push. Repo is public with branch protection, Dependabot, and secret scanning enabled.
...
- [x] **Auto-versioning** — PR merge bumps minor (v1.1.0), direct push bumps patch (v1.0.1), breaking changes tagged manually; powered by `tag` job in `ci.yml` using `GH_BROLY` PAT
- [x] **v1.0.0 tagged and released** — first stable release; GoReleaser published binaries for 4 platforms
- [x] **Branch protection** — `main` requires PR + `build` status check; force push and deletion blocked; admin bypass enabled
- [x] **Security hardening** — Dependabot alerts + security updates enabled; secret scanning + push protection enabled; all GitHub Actions pinned to commit SHAs; vulnerable dependencies upgraded
...
- [ ] Tag a release from the new org (continue semver from `Shasheen8/Broly` or restart at `v1.0.0`)
